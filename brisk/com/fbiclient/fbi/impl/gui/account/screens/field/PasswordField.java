package com.fbiclient.fbi.impl.gui.account.screens.field;
...
public class PasswordField extends Gui {
    ...
    /**
     * misc selection position, maybe the same as the cursor
     */
    ...
    public PasswordField(int componentId, FontRenderer fontrendererObj, int x, int y, int par5Width,
                            int par6Height) {
        this.id = componentId;
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            if (s.length() > 0) {
                ...
                j1 = this.fontRendererInstance.drawStringWithShadow(s1.replaceAll("(?s).", "•"), (float) l, (float) i1,
                        i);
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                j1 = this.fontRendererInstance.drawStringWithShadow(s.substring(j).replaceAll("(?s).", "•"), (float) j1,
                        (float) i1, i);
            }
            ...
            if (k != j) {
                int l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k).replaceAll("(?s).", "•"));
    ...
    /**
     * pull enable drawing background and outline
     */
